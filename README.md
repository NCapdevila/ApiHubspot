...
PORT=3001
ALIANZAS_JSON={"<api-key>":{"name":"<nombre>","agency":"<valor exacto de Productor/Agencia en HubSpot>"}}
```

- **HUBSPOT_TOKEN**: token de la Private App de HubSpot.
- **PORT**: puerto donde levanta el server (default `3001`).
- **ALIANZAS_JSON**: mapa de API keys válidas. Cada key está atada a una `agency`, que es el valor exacto de la propiedad de contacto **Productor/Agencia** en HubSpot. Una alianza solo puede consultar los datos de la agencia que tiene asignada, sin importar qué mande en el request.
...
1. Generar una key aleatoria:
   ...
   ```
2. Agregar una entrada en `ALIANZAS_JSON` con esa key y la `agency` correspondiente.
...
### `GET /deals/by-contact?email=xxx`
